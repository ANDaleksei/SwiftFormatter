...
protocol PostsFeedFacade {
  ...
  func observeActiveFeedState( ) -> Observable < PostsFeedState > func refreshCurrentStableFeed( )
  func getMoreFeed( ) -> Single < Void > func selectPlace( with id: MarkerIdentifier? )
...
final class PostsFeedFacadeImpl: PostsFeedFacade {
  ...
  init( postsProvider: PostsFeedProvider,placesProvider: PlacesProvider,filterProvider: FilterProvider ) {
    ...
    NotificationCenter.default.addObserver(self,selector: #selector( forceRefreshCurrentStableFeed ),name: YazersServiceImpl.Notifications.changeFollowing,object: nil
    )
  ...
  func observeActiveFeedState( ) -> Observable < PostsFeedState > {
    ...
    return Observable.combineLatest(allFeeds,activeFeedStateRelay.asObservable( )
    )
      ...
      return PostsFeedState(type: activeFeedType,content: activeFeedContent,activePostID: activeFeedState.activePostID
      )
    }
  ...
  func refreshCurrentStableFeed( ) {
    let request = FeedRequest(type: isBookmarkedFeed ? FeedType.bookmarked: .new,filter: filter,isBookmarked: isBookmarkedFeed
    )
  ...
  func getMoreFeed( ) -> Single < Void > {
    let request = FeedRequest(type: activeFeedStateRelay.value.activeFeed,filter: filter,isBookmarked: isBookmarkedFeed
    )
